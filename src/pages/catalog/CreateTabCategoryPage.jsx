...
import { createTabCategory, getCategories } from "../../api/catalogApi";

const toSlug = (str = "") => str.toLowerCase().trim().replace(/[^a-z0-9\s-]/g, "").replace(/\s+/g, " ").replace(/-+/g, " ");

const CreateTabCategoryPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await createTabCategory(payload);
      toast.success("Tab category created successfully");
      navigate("/tab-categories");
    } catch(error) {
      console.error("Create error:", error);
      toast.error(error.response?.data?.message || "Error creating tab category");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
      <MasterLayout>
        <Breadcrumb title="Catalog / Tab Categories / Create" />
        <div className="card h-100 p-0 radius-12">
          <div className="card-header py-16 px-24 bg-base border-0">
            <h6 className="text-lg mb-0">Create Tab Category</h6>
          </div>
          <form onSubmit={handleSubmit}>
            ...
            <div className="card-footer border-0 p-24 pt-0 d-flex justify-content-end gap-2">
              ...
              <button
                  ...
                  disabled={loading || !form.name || form.categories.length === 0}
              >
                {loading ? 'Creating...' : 'Create Tab Category'}
              </button>
            </div>
          </form>
        </div>
...
export default CreateTabCategoryPage;